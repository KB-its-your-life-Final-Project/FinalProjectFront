...
<template>
  <header class="bg-kb-yellow w-full px-[1rem] pt-[4rem] pb-[4.375rem] relative">
    <!-- 왼쪽: 뒤로가기 아이콘 or 타이틀 -->
    <div class="absolute top-[1rem] left-[1rem] flex items-center space-x-[1rem] text-kb-ui-04 text-[1.25rem]">
      <font-awesome-icon
        v-if="showBack"
        :icon="['fas', 'arrow-left']"
  ...
  {{ title }}
    ...
    <div
      v-if="showAlarm"
      class="absolute top-[1rem] right-[1rem] flex items-center space-x-[1rem] text-kb-ui-04 text-xl"
    >
      <font-awesome-icon
        :icon="['fas', 'bell']"

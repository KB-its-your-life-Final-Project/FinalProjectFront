...
import { searchRegions, type RegionData } from "@/api/regionService";

// 부모 컴포넌트로 전달할 이벤트 정의
const emit = defineEmits<{
  regionSelected: [region: RegionData];
}>();
...
const searchResults = ref<RegionData[]>([]);
const searchKeyword = ref("");
const loading = ref(false);

// 실제 API 호출 함수
const fetchRegionData = async (keyword: string): Promise<RegionData[]> => {
  try {
    // 실제 API 호출
...
const getMockData = (keyword: string): RegionData[] => {
  ...
  const sortedResults = filteredResults.sort((a, b) => {
    ...
    // 길이가 짧은 것 우선
    return a.locataddNm.length - b.locataddNm.length;
  });

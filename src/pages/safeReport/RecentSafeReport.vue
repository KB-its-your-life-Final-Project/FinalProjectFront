  ...
  <div class="relative z-0">
    ...
    >
      <div class="text-center">
        ...
        <button
          @click="router.push({ name: 'safeReport' })"
          class="bg-kb-yellow text-kb-ui-01 px-6 py-3 rounded-lg font-semibold hover:bg-yellow-500 transition-colors"
        >
          레포트 생성하기
        </button>
      </div>
    ...
    <div v-else class="p-4 h-[calc(100vh-180px)] overflow-y-auto pb-6">
      <div
        ...
        :key="report.id"
        class="bg-kb-ui-11 rounded-lg shadow-sm border border-kb-ui-06 overflow-hidden mb-4 last:mb-0"
      >
        <!-- 카드 헤더 -->
        <div class="flex items-center justify-between p-4 border-b border-kb-ui-07">
          <div class="flex items-center space-x-2">
            <span
              :class="`${getGradeColorClass(report.resultGrade)} text-sm font-bold px-3 py-2 rounded`"
            >
        ...
        >
          ...
          <div class="flex items-center justify-between pt-3 border-t border-kb-ui-07">
            ...
            <span class="text-sm font-medium text-green-600"> 안심심진단 완료 </span>
          </div>
        </div>
      </div>
    </div>
